<?php

$error = '';
$suc='';

if( isset($_POST['submit']) && $_POST['submit'] == 'contact'){
    $FirstName=(isset($_POST['FirstName']) && $_POST['FirstName']) ? $_POST['FirstName'] : "";
    $LastName =(isset($_POST['LastName ']) && $_POST['LastName ']) ? $_POST['LastName '] : "";
    $email =(isset($_POST['email']) && $_POST['email']) ? $_POST['email'] : "";
    $MobileNumber =(isset($_POST['MobileNumber']) && $_POST['MobileNumber']) ? $_POST['MobileNumber'] : "";
    $msg =(isset($_POST['msg']) && $_POST['msg']) ? $_POST['msg'] : "";

if(!$FirstName){
    $error.= "First Name is required<br>";
}
if(!$LastName){
    $error.= "First Name is required<br>";
}
if(!$email){
    $error.= "First Name is required<br>";
}
if(!$MobileNumber){
    $error.= "First Name is required<br>";
}
if(!$msg){
    $error.= "First Name is required<br>";
}
if(!$error){
    $headers="Reply-To: $email"."\r\n";
    mail("peterrubel3@gmail.com","from".$FirstName ,$msg ,$headers ,$email )
    $suc='Message Sent.';
}
    
}
if($error){
    echo $error;
}elseif($suc){
    echo $suc;
}

?>

<!doctype html>
<html>
    <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> Contact Us</title>
    <link rel="shortcut icon" type="image/jpg" href="/images/news2.png"/>
    <link rel="icon" type="image/png" sizes="32x32" href="./images/Pharmacy-logo-on-transparent-PNG.png">
    <link href="https://cdn.jsdelivr.net/npm/remixicon@3.2.0/fonts/remixicon.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" integrity="sha512-SnH5WK+bZxgPHs44uWIX+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkcmfRyVX3pBnMFcV7oQPJkl9QevSCWr3W6A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <!--=============== CSS ===============-->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-C6RzsynM9kWDrMNeT87bh95OGNyZPhcTNXj1NW7RuBCsyN/o0jlpcV8Qyq46cDfL" crossorigin="anonymous"></script>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@200&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@500&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@600&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@700&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@800&display=swap');
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@900&display=swap');
    </style>
    <link rel="stylesheet" href="./style.css">
    <script src="./index.js"></script>
    
    </head>
    <body>
        <header class="header">
            <nav class="nav container">
               <div class="nav__data">
                  <a href="#" class="nav__logo">
                     <img src="./images/Pharmacy-logo-on-transparent-PNG.png" alt=""> Optavit
                  </a>

                  
                  <div class="nav__toggle" id="nav-toggle">
                     <i class="ri-menu-line nav__burger"></i>
                     <i class="ri-close-line nav__close"></i>
                  </div>
               </div>
    
               <!--=============== NAV MENU ===============-->
               <div class="nav__menu" id="nav-menu">
                  <ul class="nav__list">
                     <li><a href="../index.html" class="nav__link">Home</a></li>
    
                     <li><a href="../index.html" class="nav__link">Company</a></li>
    
                     <!--=============== DROPDOWN 1 ===============-->
                     
                     
                     <li><a href="../Shopping-Cart-master/index.html" class="nav__link">Products</a></li>
    
                     <!--=============== DROPDOWN 2 ===============-->
                     
    
                     <li><a href="../Contact-Us-Page-Design-master/index.html" class="nav__link">Contact us</a></li>
                     <li><a href="../Sliding-Sign-In-Sign-Up-Form-master/index.html" class="nav__link">sign up</a></li>
                     <li><a href="../Sliding-Sign-In-Sign-Up-Form-master/index.html" class="nav__link">sign in</a></li>
                  </ul>
               </div>
            </nav>
         </header>
        <section>
            <div class="contactus">
                <div class="contactInfo"> 
                    <div>
                        <h2>Contact Info</h2>
                        <ul class="info">
                            <li>
                                <span><img src="./images/location.png"></span>
                                <span>184 Ippokratous Street<br>
                                    Athens, Gr<br>
                                    11472</span>
                                </span>
                            </li>
                            <li>
                                <span><img src="./images/mail.png"></span>
                                <!-- <span>nassosanagn@gmail.com</span> -->
                                <span><a href = "mailto: nassosanagn@gmail.com">nassosanagn@gmail.com</a></span>
                            </li>
                            <li>
                                <span><img src="./images/call.png"></span>
                                <span>702-279-3488</span>
                            </li>

                        </ul>
                    </div>
                    <ul class="sci">
                        <li><a href="https://www.facebook.com/nassosanagn/"><img src="./images/1.png"></a></li>
                        <li><a href="https://www.instagram.com/nassosanagn_/?hl=el"><img src="./images/3.png"></a></li>
                        <li><a href="https://twitter.com/nassosanagn"><img src="./images/2.png"></a></li>
                        <li><a href="https://www.linkedin.com/in/nassos-anagnostopoulos-2b9631196/"><img src="./images/5.png"></a></li>
                        
                    </ul>
                </div>
                <form action="./index.php" method="POST">
                    <div class="contactForm">
                        <h2>Send a Message</h2>
                        <div class="formBox">
                        <div class="inputBox w50">
                            <input type="text" name="FirstName" value="<?php $FirstName; ?>" required>
                            <span>First Name</span>
                        </div>
                        <div class="inputBox w50">
                            <input type="text" name="LastName" value="<?php $LastName; ?>" required>
                            <span>Last Name</span>
                        </div>
                        <div class="inputBox w50">
                            <input type="email" name="email" value="<?php $email; ?>" required>
                            <span>Email Address</span>
                        </div>
                        <div class="inputBox w50">
                            <input type="text" name="MobileNumber" value="<?php $MobileNumber; ?>" required>
                            <span>Mobile Number</span>
                        </div>
                        <div class="inputBox w100">
                            <textarea name="msg" value="<?php $msg; ?>" required></textarea>
                            <span>Write your message here...</span>
                        </div>
                        <div class="inputBox w100">
                           <input type="submit" name="submit" value="Send">
                        </div>
                    </div>
                </div>
                    
            </form> 
             
        </section>
    </body>
</html>